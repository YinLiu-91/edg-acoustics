...
The TileLang derivative-volume AoS path is the replacement experiment for that per-output Triton kernel. It computes `Dr/Ds/Dt @ Q` as GEMM-like tiles, reuses the same `Q` shared tile for all three derivative matrices, and applies the affine volume/surface epilogue before writing RHS. It is forced-only because standalone derivative TileLang GEMM on C500 was measured at about `0.97x` of mcBLAS for the large derivative shape, so any win must come from fusing away the intermediate `dQ` global memory traffic. The current reference fused config is `bp16_be8_bn32_bk16_s0_t128_fullcol`, with about `1.025x` micro-kernel speedup over derivative+volume baseline on `scenario1_profile_lc0p20.msh`; this is below the default-enable threshold, so `auto` remains disabled. Runtime pinning still uses `EDG_ACOUSTICS_TILELANG_DERIVATIVE_VOLUME_AOS_CONFIG`, while offline tuning now lives in `benchmarks/tilelang_derivative_volume_aos_autotune.py`. Default autotune is restricted to the known-correct `copy_shared` variant. The `direct_epilogue` and `merged3` variants are excluded from default autotune; `merged3` can win TileLang's raw latency benchmark while failing the final RHS/state correctness check, so request these variants explicitly only for debugging.
...
After those experiments, the accepted MetaX-specific derivative path is no longer "future work": a fixed TileLang derivative GEMM backend now exists for the merged derivative shape `D_merged[105, 35] @ Q_flat[35, N]`. The current pinned runtime config is `bm128_bn64_bk4_s0_t256_fullcol`, promoted from a focused C500 sweep because it beats the previous `bm112_bn64_bk12_*` family at the true timestep-like `N=181140` and also wins at larger `N`. The key gain is shrinking `block_K` from `12` to `4` for the skinny `K=35` case while keeping `block_N=64` and `threads=256`; this cuts K-side wasted work enough that the larger `block_M=128` padding still wins overall. On `scenario1_profile_lc0p20.msh` with TileLang lift enabled, the new pinned config now reduces the full-step runtime from `7.368161 ms/step` to `6.709760 ms/step`, an end-to-end speedup of about `1.098x`; this is also about `1.78%` faster than the earlier `bm112_bn64_bk12_s1_t256_fullcol` full-step result of `6.829143 ms/step`. See `docs/tilelang_derivative_gemm_c500.md` for the detailed sweep evidence, configuration rationale, and exact benchmark commands.

Recommended offline tuning flow on a C500 box:

```bash
python benchmarks/tilelang_derivative_volume_aos_autotune.py \
  --mesh-name scenario1_profile_lc0p20.msh \
  --mode update \
  --profile-backend cudagraph
```

Use `--list-candidates` to inspect the filtered search space, `--save-inputs` to persist real scenario tensors for repeatable standalone profiling, and then promote the chosen runtime config with `EDG_ACOUSTICS_TILELANG_DERIVATIVE_VOLUME_AOS_CONFIG=<best-config>`.
