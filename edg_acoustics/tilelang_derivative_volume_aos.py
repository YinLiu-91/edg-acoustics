...
def _fp64_derivative_volume_aos(
            ...
            with T.Kernel(
                ...
                for ko in T.Pipelined(T.ceildiv(K, block_k), num_stages=num_stages):
                    for i, kk in T.Parallel(block_p, block_k):
                        ...
                        k_idx = ko * block_k + kk
                        valid = (row < M) & (k_idx < K)
                        dr_shared[i, kk] = T.if_then_else(valid, Dr[row, k_idx], T.float64(0.0))
                        ds_shared[i, kk] = T.if_then_else(valid, Ds[row, k_idx], T.float64(0.0))
                        dt_shared[i, kk] = T.if_then_else(valid, Dt[row, k_idx], T.float64(0.0))
                    ...
                    T.gemm(dr_shared, q_shared, acc_r, transpose_B=True, policy=policy)
                    T.gemm(ds_shared, q_shared, acc_s, transpose_B=True, policy=policy)
                    T.gemm(dt_shared, q_shared, acc_t, transpose_B=True, policy=policy)
    ...
    if variant == _VARIANT_FIELD_FRAGMENTS:
        @T.prim_func
        def main(
            Q: T.Tensor((M, N), T.float64),
            Dr: T.Tensor((M, K), T.float64),
                ...
                for ko in T.Pipelined(T.ceildiv(K, block_k), num_stages=num_stages):
                    ...
                    for field_offset in range(4):
                        for j, kk in T.Parallel(field_n, block_k):
                            ...
                            valid = (j < block_e) & (elem < n_tets) & (k_idx < K) & (n_idx < N)
                            q_field_shared[j, kk] = T.if_then_else(valid, Q[k_idx, n_idx], T.float64(0.0))

                        if field_offset == 0:
                            T.gemm(dr_shared, q_field_shared, p_r_acc, transpose_B=True, policy=policy)
                            T.gemm(ds_shared, q_field_shared, p_s_acc, transpose_B=True, policy=policy)
                            T.gemm(dt_shared, q_field_shared, p_t_acc, transpose_B=True, policy=policy)
                        elif field_offset == 1:
                            T.gemm(dr_shared, q_field_shared, vx_r_acc, transpose_B=True, policy=policy)
                            T.gemm(ds_shared, q_field_shared, vx_s_acc, transpose_B=True, policy=policy)
                            T.gemm(dt_shared, q_field_shared, vx_t_acc, transpose_B=True, policy=policy)
                        elif field_offset == 2:
                            T.gemm(dr_shared, q_field_shared, vy_r_acc, transpose_B=True, policy=policy)
                            T.gemm(ds_shared, q_field_shared, vy_s_acc, transpose_B=True, policy=policy)
                            T.gemm(dt_shared, q_field_shared, vy_t_acc, transpose_B=True, policy=policy)
                        else:
                            T.gemm(dr_shared, q_field_shared, vz_r_acc, transpose_B=True, policy=policy)
                            T.gemm(ds_shared, q_field_shared, vz_s_acc, transpose_B=True, policy=policy)
                            T.gemm(dt_shared, q_field_shared, vz_t_acc, transpose_B=True, policy=policy)
        ...
        with T.Kernel(
            ...
            for ko in T.Pipelined(T.ceildiv(K, block_k), num_stages=num_stages):
                ...
                for pair_index in range(2):
                    for j, kk in T.Parallel(pair_n, block_k):
                        ...
                        valid = (local_elem < block_e) & (elem < n_tets) & (k_idx < K) & (n_idx < N)
                        q_pair_shared[j, kk] = T.if_then_else(valid, Q[k_idx, n_idx], T.float64(0.0))

                    if pair_index == 0:
                        T.gemm(dr_shared, q_pair_shared, pair0_r_acc, transpose_B=True, policy=policy)
                        T.gemm(ds_shared, q_pair_shared, pair0_s_acc, transpose_B=True, policy=policy)
                        T.gemm(dt_shared, q_pair_shared, pair0_t_acc, transpose_B=True, policy=policy)
                    else:
                        T.gemm(dr_shared, q_pair_shared, pair1_r_acc, transpose_B=True, policy=policy)
                        T.gemm(ds_shared, q_pair_shared, pair1_s_acc, transpose_B=True, policy=policy)
                        T.gemm(dt_shared, q_pair_shared, pair1_t_acc, transpose_B=True, policy=policy)
            ...
            for i, e in T.Parallel(block_p, block_e):
                ...
                if node < M and elem < n_tets:
                    ...
                    p_t = pair0_t_shared[i, pair_col]
                    vx_r = pair0_r_shared[i, pair_col + 1]
                    vx_s = pair0_s_shared[i, pair_col + 1]
                    vx_t = pair0_t_shared[i, pair_col + 1]
                    vy_r = pair1_r_shared[i, pair_col]
                    vy_s = pair1_s_shared[i, pair_col]
                    vy_t = pair1_t_shared[i, pair_col]
                    vz_r = pair1_r_shared[i, pair_col + 1]
                    vz_s = pair1_s_shared[i, pair_col + 1]
                    vz_t = pair1_t_shared[i, pair_col + 1]
