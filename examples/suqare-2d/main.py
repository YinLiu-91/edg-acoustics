...
mesh_name = "4-elem-tri.msh"  # name of the mesh file. The mesh file should be in the same folder as this script.
# mesh_name = "scenario1_coarser.msh"  # name of the mesh file. The mesh file should be in the same folder as this script.
monopole_xyz = numpy.array([0.5, 0.5, 0.0])  # x,y,z coordinate of the source in the room
...
recx = numpy.array([0.75])
recy = numpy.array([0.5])
recz = numpy.array([0.0])
...
sim.init_IC(IC)
sim.init_Flux(flux)
sim.init_rec(rec, "scipy")  # brute_force or scipy(default) approach to locate the receiver points in the mesh

use_triton = False
if use_triton:
    # operator = sim.RHS_operator_triton_kernel
    operator = sim.RHS_operator_triton_fused_kernel
else:
